...
use App\Models\Domain; // Add this import
    ...
    public function approve($id)
    {
        try {
            ...
            if (empty($pending->name)) {
                throw new \RuntimeException('The company_name attribute is missing in the pending tenant.');
            }

            // Create a new tenant in the tenants table
            $tenant = Tenant::create([
                'id' => $pending->name, // Use the company name as the tenant ID
                'data' => [
                    'tenancy_db_name' => 'tenant_' . strtolower($pending->name),
                    'company_name' => $pending->name, // Explicitly set company_name
                ],
            ]);

            Log::info('Tenant created successfully', ['tenant' => $tenant->toArray()]);

            // Create a new domain entry in the domains table
            $domain = Domain::create([
                'domain' => $pending->domain,
                'tenant_id' => $tenant->id,
            ]);

            Log::info('Domain created successfully', ['domain' => $domain->toArray()]);

            // Approve the tenant and create the database
            $this->tenantService->approveTenant($tenant);

            // Mark the pending tenant as approved
            $pending->update([
                'approved' => true,
                'database_name' => 'tenant_' . strtolower($pending->name),
            ]);

            Log::info('Tenant approved successfully', [
                'tenant_id' => $tenant->id,
                'domain' => $domain->domain,
            ]);

            return back()->with('success', 'Tenant approved and added to the system!');
        } catch (\RuntimeException $e) {
            Log::error('Runtime error during approval: ' . $e->getMessage());
